...
// Microsoft Visual C++ is not C99 compatible so this code has to be compiled as C++ code
// Using a custom stdint.h leads to problems with boost (ambiguous symbol), so why don't we
// just use the boost provided one?
 #include <boost/cstdint.hpp>
 #include <Winsock2.h>  // ntohl, etc
 #include <stdio.h> // _snprinf
 #define snprintf _snprintf
 #define __thread __declspec(thread) // thread local (TLS) variables
 // pull boost types (in the name of compatibility)
 using boost::uint8_t;
...
# define smpp34_errno			(TlsGetValue(dwTlsIndex) ? ((TlsInfo *)TlsGetValue(dwTlsIndex))->_errno : 0)
# define smpp34_strerror		(TlsGetValue(dwTlsIndex) ? ((TlsInfo *)TlsGetValue(dwTlsIndex))->_strerror : "")
# define smpp34_error_size    2048

# define smpp34_err_init() do { \
	TlsInfo *tlsInfo = (TlsInfo *)TlsGetValue(dwTlsIndex); \
	if (tlsInfo == NULL) { \
		tlsInfo = (TlsInfo *)LocalAlloc(LPTR, sizeof(TlsInfo)); \
		TlsSetValue(dwTlsIndex, (LPVOID)tlsInfo); \
	} else { \
		memset(tlsInfo, 0, sizeof(TlsInfo)); \
	} \
	tlsInfo->_ptrerror = tlsInfo->_strerror; \
} while (0)

#else

	extern __thread int smpp34_errno;
	extern __thread char *ptrerror;
	extern __thread char smpp34_strerror[2048];
	...
	#define smpp34_error_size sizeof(smpp34_strerror)
